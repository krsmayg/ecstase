...
const WallCollections = React.memo((props) => {
  ...
  const collections = props.collections.map((col) => {
    ...
    const posters = col.posters.map((poster) => (
      <div
        className="wall-poster-item"
        style={{ width: "33.3%", padding: "0 14px", height: "100%" }}
      >
        <div
          className="wall-poster-item__box"
          onClick={() => goToPosterDetail(poster.slug)}
        >
    ...
    return (
      <div className="collection-item">
        <img
          src={`${imageUrl}/collections/${col.imageCollection}`}
          style={{ width: "100%", objectFit: "cover" }}
        />
        <div
          className="wall-poster-box"
          style={{
            position: "absolute",
            display: "flex",
            top: "19%",
...
export default connect(mapStateToProps, { fetchCollections })(
  withRouter(WallCollections)
);
